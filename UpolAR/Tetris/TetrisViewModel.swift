...
class TetrisViewModel: ViewModel {
    ...
    func gestureRecognizer(value: DragGesture.Value) {
        if value.translation.width < -50 { horizontalMove(horizontalMove: .left) }
        if value.translation.width > 50 { horizontalMove(horizontalMove: .right) }
        if value.translation.height < -50 { rotate() }
        if value.translation.height > 50 { renderGame() }
    }
    ...
    private func clearNextBoard() {
        for (rowIndex, _) in nextBoard.enumerated() {
            for (columnIndex, value) in nextBoard[rowIndex].enumerated() {
                if value == 1 { nextBoard[rowIndex][columnIndex] = 0 }
            }
        }
    }
    ...
    private func isNextBoardValid() -> Bool {
        ...
        for (rowIndex, _) in nextBoard.enumerated() {
            for (columnIndex, value) in nextBoard[rowIndex].enumerated() {
                if value == 1 && board[rowIndex][columnIndex] == 2 {
                    valid = false
                    return valid
                }
            }
