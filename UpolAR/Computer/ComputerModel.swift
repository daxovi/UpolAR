...
class ComputerModel {
    ...
    func getModel() -> AnchorEntity {
        ...
        if let videoMaterial = loadVideoMaterial() { videoBox.model?.materials = [videoMaterial] }
    ...
    private func loadVideoMaterial() -> VideoMaterial? {
        guard let pathToVideo = Bundle.main.path(forResource: videoName, ofType: "mov") else { return nil }
